...
- **基準年**: 2026
- **1ベース**: 週インデックス 1 = 2026年1月第1週
- **年をまたぐ絶対値**: `週インデックス = (年 - 基準年) * 52 + 月別開始インデックス[月] + 週番号(0-based)`

月ごとの開始インデックス（年内、1月起点）:
...
DBに格納される `開始週`・`終了週` は上記インデックス体系の絶対値（基準年2026の場合は1〜52）。

#### 2.2.5 Googleカレンダーエクスポート仕様

**週の定義**
「M月第N週」= M月のN番目の土曜日がある週（週はその土曜日で識別する）。

**週インデックス → 実日付の変換**
`週インデックス` → `{年, 月, 週番号(0-based)}` → `M月第(週番号+1)土曜日`

M月に第N土曜日が存在しない場合（第5土曜日がない月など）は、翌月第1土曜日を使用する。

```
土曜日を取得する(年, 月, 週番号):
  n = 週番号 + 1
  firstSaturday = 1 + (6 - new Date(年, 月-1, 1).getDay() + 7) % 7
  nthSaturday = firstSaturday + (n-1) * 7
  nthSaturdayがM月内に存在しない場合 → 翌月第1土曜日を再帰呼び出し
```

**Googleカレンダーへの登録形式**
- カレンダー名: `hatakei`（存在しなければ新規作成。存在する場合は削除→再作成して全件クリア）
- イベントタイトル: `{野菜名},{イベント名}`
- 日時: 土曜日〜月曜日の終日イベント（`start.date`=土曜日, `end.date`=月曜日 ※exclusive）
- 説明: イベントのコメント

**年またぎの処理**
イベントの週インデックスはDB格納時点の年（例: 2026年は1〜52）に固定されているが、エクスポート範囲が年境界をまたぐ場合（例: 2026年3月〜2027年2月）、1月〜2月のイベントを翌年として扱う必要がある。

`k*52` シフトにより解決する:

```
baseK = floor((exportStart - 1) / 52)
k in [baseK, baseK+1] を試行:
  調整開始週 = ev.開始週 + k * 52
  調整終了週 = ev.終了週 + k * 52
  重複区間 = [max(調整開始週, exportStart), min(調整終了週, exportEnd)]
  重複があれば、その週をエクスポート対象に追加
```

例: エクスポート範囲 2026年3月〜2027年2月（インデックス 10〜61）
- 1月イベント（インデックス 1〜4）: k=1 でシフト → 53〜56（2027年1月）✓
- 3月イベント（インデックス 10〜13）: k=0 のまま → 10〜13（2026年3月）✓
