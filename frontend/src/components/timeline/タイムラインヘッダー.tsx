...
export const 野菜名ヘッダー = ({ スケジュール一覧, タイムラインid }: Props) => {
  ...
  const ヘッダーをクリック = (e: React.MouseEvent, schedule: スケジュール) => {
    // ドラッグ後のクリックを無視するため、mousemoveが0pxの場合のみ
    e.stopPropagation();
    ...
    <div className="flex relative">
      {スケジュール一覧.map((schedule, i) => {
        ...
        return (
          <div
            ...
            onDragEnd={dragEnd}
            onDrop={() => drop(i)}
            onClick={(e) => ヘッダーをクリック(e, schedule)}
      ...
      {ポップアップ && (
        <>
          ...
          />
          <div
            className="absolute top-full left-0 z-50 bg-white border border-gray-200 rounded shadow-lg p-3 w-52"
